...
class BE(ReportTestResult, PATH):
    img_path = 'img/' if platform.system() != 'Windows' else 'img\\'

    def __init__(self, suites):
        super(BE, self).__init__(suites)
        self.suites = suites
    ...
    def output_report(self, theme):
        ...
        res = json.loads(render_params["resultData"])
        EnvData.testName = jsonpath.jsonpath(res, "$.testName")[0]
        EnvData.testPass = jsonpath.jsonpath(res, "$.testPass")[0]
        EnvData.testAll = jsonpath.jsonpath(res, "$.testAll")[0]
        EnvData.testFail = jsonpath.jsonpath(res, "$.testFail")[0]
        EnvData.testError = jsonpath.jsonpath(res, "$.testError")[0]
        EnvData.beginTime = jsonpath.jsonpath(res, "$.beginTime")[0]
    ...
    def add_test_img(*pargs):
        ...
        def _wrap(func):
            ...
            def __wrap(*args, **kwargs):
                img_path = os.path.abspath('{}'.format(BE.img_path))
                try:
                    ...
                    if 'save_img' in dir(args[0]):
                        ...
                        data = BE.img2base(img_path, pargs[0] + '.png')
                ...
                if len(pargs) > 1:
                    for parg in pargs:
                        print(parg + ':')
                        data = BE.img2base(img_path, parg + '.png')
                        print(HTML_IMG_TEMPLATE.format(data, data))
                    return result
